...
# macOS exposes USB serial as /dev/cu.*; Linux as /dev/ttyUSB*/ttyACM*.
# Both are tried on every platform so dual-host setups (e.g. macOS host with
# Linux container forwarding) still work.
_PORT_PATTERNS_LINUX = ('/dev/ttyUSB*', '/dev/ttyACM*')
_PORT_PATTERNS_MACOS = (
    '/dev/cu.usbserial-*',
    '/dev/cu.wchusbserial*',
...
def _candidate_port_patterns() -> tuple[str, ...]:
    """Return glob patterns appropriate for the host OS (macOS-first on Darwin)."""
    if sys.platform == 'darwin':
        return _PORT_PATTERNS_MACOS + _PORT_PATTERNS_LINUX
    return _PORT_PATTERNS_LINUX + _PORT_PATTERNS_MACOS
...
def find_all_waveshare_ports() -> list[str]:
    ...
    Scans both Linux (`/dev/ttyUSB*`, `/dev/ttyACM*`) and macOS
    (`/dev/cu.usbserial-*`, `/dev/cu.wchusbserial*`, `/dev/cu.SLAB_USBtoUART*`)
    patterns.
    ...
    found: list[str] = []
    seen: set[str] = set()
    for pattern in _candidate_port_patterns():
